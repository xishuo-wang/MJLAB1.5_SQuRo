...
# XML文件路径
MOUSE_XML: Path = (
    MJLAB_SRC_PATH / "asset_zoo" / "robots" / "mouse" / "xmls" / "Mouse.xml"
)
assert MOUSE_XML.exists()


def get_assets(meshdir: str) -> dict[str, bytes]:
    assets: dict[str, bytes] = {}
    update_assets(assets, MOUSE_XML.parent / "assets", meshdir)
    return assets


def get_spec() -> mujoco.MjSpec:
    spec = mujoco.MjSpec.from_file(str(MOUSE_XML))
    spec.assets = get_assets(spec.meshdir)
    return spec


# 执行器配置
MOUSE_ARTICULATION = EntityArticulationInfoCfg(
    actuators=(
        XmlActuatorCfg(
            target_names_expr=(
...
# 机器人配置
def get_mouse_robot_cfg() -> EntityCfg:
  return EntityCfg(
    init_state=INIT_STATE,
    spec_fn=get_spec,
    articulation=MOUSE_ARTICULATION,
  )
...
if __name__ == "__main__":
    ...
    robot = Entity(get_mouse_robot_cfg())
