# Viz_Slalom_Robot: Slalom 任务简化机器人模型 (XoY 俯视)
# 结构 (三块长方形, 从前往后): 头部 → 前躯干及前脊柱 → 后躯干及后脊柱
# 偏航关节共 2 个, 位于两两相接的边的中点:
#   关节1: 头部与前躯干相接边中点
#   关节2: 前躯干与后躯干相接边中点 (= base 位置)
# 运动学 (相对角链式累积): 后躯干=0, 前躯干=JOINT2, 头部=JOINT2+JOINT1
# 在 XoY 平面中矩形绕关节旋转 = 偏航 (绕 Z 轴), 与 Tunnel 的 XoZ 俯仰对称
import numpy as np
...
HEAD_WIDTH_MM = 50       # 头部宽度 (mm, 左右方向 Y)
FRONT_LENGTH_MM = 100    # 前躯干及前脊柱长度 (mm)
FRONT_WIDTH_MM = 50      # 前躯干及前脊柱宽度 (mm)
REAR_LENGTH_MM = 80      # 后躯干及后脊柱长度 (mm)
REAR_WIDTH_MM = 50       # 后躯干及后脊柱宽度 (mm)
# 提示: 若需对齐 Slalom corridor 半宽 0.035, 可将宽度改为 70mm (半宽 35mm)

# --- 偏航关节控制角度 (单位: 度, 共 2 个关节各对应一个角度; 0 = 直行) ---
JOINT1_YAW_DEG = 0.0     # 关节1 相对角: 头部相对前躯干的偏航 (绕关节1)
JOINT2_YAW_DEG = 15.0    # 关节2 相对角: 前躯干相对后躯干的偏航 (绕关节2, 后躯干保持直行)
...
# --- 输出 ---
SAVE_PATH = Path(__file__).parent / "Result" / "Viz_Slalom_Robot.png"
...
def rect_points(center, half_l: float, half_w: float, yaw: float,
...
def draw_slalom_robot(ax) -> None:
    ...
    head_pts = rect_points(head_center, HEAD_LENGTH_MM * mm / 2, HEAD_WIDTH_MM * mm / 2,
                           j1_yaw + j2_yaw, j1_init, j1_new - j1_init)
    front_pts = rect_points(front_center, FRONT_LENGTH_MM * mm / 2, FRONT_WIDTH_MM * mm / 2,
                            j2_yaw, j2)
    rear_pts = rect_points(rear_center, REAR_LENGTH_MM * mm / 2, REAR_WIDTH_MM * mm / 2,
                           0.0, j2)  # 后躯干保持直行
    ...
    for j, name in [
        ...
        ax.add_patch(
            Circle((float(j[0]), float(j[1])), JOINT_RADIUS, facecolor="white",
                   edgecolor=COLOR_BASE, linewidth=1.5, zorder=6)
        )
        ax.annotate(name, (float(j[0]), float(j[1])), textcoords="offset points",
                    xytext=(0, 12), ha="center", fontsize=8, color=COLOR_BASE)
    ...
    ax.annotate("", xy=(float(arrow_tip[0]), float(arrow_tip[1])),
                xytext=(float(BASE_X), float(BASE_Y)),
                arrowprops=dict(arrowstyle="->", color="black", lw=1.5))
    ax.text(float(arrow_tip[0]), float(arrow_tip[1]) + 0.003, "前进方向",
