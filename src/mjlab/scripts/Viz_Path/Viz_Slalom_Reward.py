"""绘制走廊一致性 (Corridor Conformance) 奖励示意图 (v3)
更新:
  - 弧角范围增至 ±0.95π，模拟绕杆近一周
  - 整图顺时针旋转 90°
  - 保留无坐标轴、无网格、黄→紫渐变惩罚配色
用法:
  python plot_corridor_diagram.py [--radius 0.15] [--delta_theta 28] [--d_lat 0.008] [--spine_angle 12] [--out xxx.png]
"""
import argparse
import numpy as np
...
# ===== 真实尺寸参数 =====
CORRIDOR_C = 0.04   # 走廊半宽
BODY_L = 0.04       # 身体半长
BODY_W = 0.035      # 身体半宽
OFFSET = 0.04       # F/H 中心距 base 偏移

THETA_RANGE = (-np.pi * 0.95, np.pi * 0.95)   # 接近一整圈，留小缺口
N_SAMPLE = 60
...
def main():
    ...
    # ---- 路径与走廊带（原始坐标） ----
    theta = np.linspace(*THETA_RANGE, 800)  # 增加采样密度，使圆弧更平滑
    ...
    # 3. 脊柱弧线
    ax.plot(spine_pts_x, spine_pts_y, '-', color='#555555', linewidth=2.0, alpha=0.9, zorder=5)

    # 4. 躯干（金色半透明，0惩罚）
    ax.add_patch(Polygon(np.column_stack([f_rect_x, f_rect_y]),
                         closed=True, facecolor='#FFD700', edgecolor='#B8860B',
                         linewidth=1.6, alpha=0.4, zorder=4))
    ax.add_patch(Polygon(np.column_stack([h_rect_x, h_rect_y]),
                         closed=True, facecolor='#FFD700', edgecolor='#B8860B',
                         linewidth=1.6, alpha=0.4, zorder=4))

    # 5. 脊柱关节（深灰圆形）
    ax.add_patch(Circle((base_x, base_y), radius=0.012, facecolor='dimgrey', edgecolor='black',
    ...
    if out_x:
        ...
        sc = ax.scatter(out_x_rot, out_y_rot, s=18, c=out_dist, cmap='plasma_r',
