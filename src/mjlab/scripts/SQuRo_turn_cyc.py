r"""
SQuRo 原地转弯 — 摆线足端轨迹 + 逆运动学（MJLAB 1.5 移植版）

迁移自 D:\Code\SQuRo-MuJoCo\Loco_Turn\Trun_Cyc_Min.py
使用 MJLAB 1.5 的 SQuRo.xml 模型 + MuJoCo 内置位置执行器。
足端轨迹由摆线生成，关节角度由逆运动学解算，脊柱侧摆实现转弯。
记录 CSV 供 CSV_Anaylsis.py 分析。
"""
...
def cycloid_trajectory(t_mod, T_sw, T_st, S, H, body_height, rotation_angle, x_offset, is_front=True):
    ...
    Args:
        ...
        S: 步长
        H: 抬脚高度
        body_height: 身体高度（正值向下）
        rotation_angle: 旋转角（弧度）
        x_offset: X方向偏移
        is_front: 是否为前腿
    Returns:
        (x_rot, z_rot, x_rot, z_rot): 旋转后的足端坐标
    """
    if t_mod <= T_sw:
        # 摆动相：摆线轨迹
        t_norm = t_mod / T_sw
...
def inverse_kinematics(target_pos, is_front=True):
    """逆运动学解算（2 连杆机构 → 髋/肩 + 膝/肘角度）

    Args:
        target_pos: (x, y) 足端目标位置（在腿的局部坐标系中，y = z/高度方向）
        is_front: 前腿=True, 后腿=False
    Returns:
        (hip_angle, knee_angle): 弧度
    """
    x, y = target_pos
...
# 脊柱控制（由 TurnConfig.spine_yaw 覆盖）
# 控制频率（由 TurnConfig.high_level_dt 覆盖）
...
# 腿关节在 12 个执行器中的索引
LEG_JOINT_MAP = {
    "FL": {"hip": 0, "knee": 1, "is_front": True},
    "FR": {"hip": 2, "knee": 3, "is_front": True},
    "HL": {"hip": 4, "knee": 5, "is_front": False},
    "HR": {"hip": 6, "knee": 7, "is_front": False},
}


class TurnDataRecorder:
    """记录关节数据到 CSV，兼容 CSV_Anaylsis.py"""
    ...
    def record_step(self, data: mujoco.MjData, target_pos: np.ndarray):
        ...
        for i, name in enumerate(CSV_JOINT_NAMES):
            ...
            # 位置（相对于默认关节位置）
            record[f"{name}_pos"] = float(data.qpos[qpos_adr] -
                                          self.model.qpos0[qpos_adr])
            # 速度
            record[f"{name}_vel"] = float(data.qvel[dof_adr])
            # 加速度
            record[f"{name}_acc"] = float(data.qacc[dof_adr])
            # 力矩（执行器力）
            record[f"{name}_torque"] = float(data.qfrc_actuator[dof_adr])
            # 动作（目标位置）
            record[f"{name}_action"] = float(target_pos[i])
            # 参考位置/速度（占位）
            record[f"{name}_ref_pos"] = float(target_pos[i])
            record[f"{name}_ref_vel"] = 0.0

        # 基座位置
        base_pos = data.xpos[self._base_body_id]
        record["base_pos_x"] = float(base_pos[0])
        record["base_pos_y"] = float(base_pos[1])
        record["base_pos_z"] = float(base_pos[2])

        # 基座线速度（世界系）
        base_lin_vel = data.cvel[self._base_body_id][3:6]  # 线速度部分
        record["base_lin_vel_x"] = float(base_lin_vel[0])
        record["base_lin_vel_y"] = float(base_lin_vel[1])
        record["base_lin_vel_z"] = float(base_lin_vel[2])

        # 基座角速度（世界系）
        base_ang_vel = data.cvel[self._base_body_id][:3]  # 角速度部分
        record["base_ang_vel_x"] = float(base_ang_vel[0])
        record["base_ang_vel_y"] = float(base_ang_vel[1])
        record["base_ang_vel_z"] = float(base_ang_vel[2])
        ...
        record["heading"] = float(np.arctan2(sin_h, cos_h))
        ...
        record["f_body_heading"] = float(np.arctan2(sin_h_fb, cos_h_fb))

        # 足端位置（世界系）
        for i, name in enumerate(FOOT_NAMES):
            site_id = self._foot_site_ids[i]
            site_pos = data.site_xpos[site_id]
            record[f"foot_{name}_x"] = float(site_pos[0])
            record[f"foot_{name}_y"] = float(site_pos[1])
            record[f"foot_{name}_z"] = float(site_pos[2])

        # 接触力（无传感器，置 0）
        for name in FOOT_NAMES:
            for axis in ["x", "y", "z"]:
                record[f"contact_{name}_{axis}"] = 0.0
            record[f"contact_{name}_mag"] = 0.0

        # 命令参数（固定值，适配转弯场景）
        record["vel_command_x"] = 0.0        # 原地转弯，前进速度为 0
        record["height_f_command"] = 0.06
        record["height_h_command"] = 0.06
        record["gait_freq_command"] = FREQ
        record["curvature_command"] = 2.0    # 高曲率（近似原地转弯）
...
def set_initial_pose(model: mujoco.MjModel, data: mujoco.MjData):
    ...
    以模型编译后的 qpos0 为基础（包含正确的自由关节四元数），
    仅覆盖执行器和闭链被动关节的位置。
    """
...
def run_simulation(model: mujoco.MjModel, data: mujoco.MjData,
                   recorder: "TurnDataRecorder", default_ctrl: np.ndarray,
                   cfg: TurnConfig):
    """运行仿真循环（headless 或带查看器）"""
    ...
    def simulation_step():
        ...
        if current_time - last_high_level_time >= cfg.high_level_dt or current_time == 0.0:
            ...
            if current_time > cfg.start_time:
                ...
                # 脊柱控制：侧摆实现转弯
                target[8] = cfg.spine_yaw   # F_spine1 → 侧摆
                target[9] = 0.0              # F_body → 不扭转
                target[10] = 0.0             # H_spine1 → 保持中立
                target[11] = 0.0             # H_body → 不扭转
        ...
        # 进度输出
        if step_count % 500 == 0:
            print(f"[PROGRESS] t={data.time:.2f}s / {cfg.total_time}s "
                  f"({step_count} steps)")
...
def main():
    ...
    # 运行仿真
    try:
        initial_x, initial_y, initial_heading = run_simulation(
            model, data, recorder, default_ctrl, cfg)
    ...
    if recorder.records:
        ...
        if len(motion_df) > 0:
            ...
            print(f"初始基座: ({initial_x:.4f}, {initial_y:.4f})")
