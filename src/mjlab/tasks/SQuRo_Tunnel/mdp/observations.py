...
def heading(env: ManagerBasedRlEnv, asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG) -> torch.Tensor:
    return get_f_body_physical_heading(env).unsqueeze(-1)
...
# 路径参考: [前肢组高度误差, 后肢组高度误差, 前肢期望高度, 后肢期望高度]
def path_ref(env: ManagerBasedRlEnv, asset_cfg: SceneEntityCfg = _DEFAULT_ASSET_CFG) -> torch.Tensor:
    front_z_ref, rear_z_ref, front_err, rear_err = compute_path_ref(env)
    return torch.stack([front_err, rear_err, front_z_ref, rear_z_ref], dim=1)  # [N, 4]
