...
# 所以 +X 墙必须同步内收。2026-09-26 第二批把 +X 固定值从 +0.055 收到 **+0.05**
# (内侧 +0.040), 使**末档两侧对称**: (−0.05, +0.05) ⇒ 内壁 ±0.04, 净宽 **0.080**,
# 直接用来探极限宽度。代价是 reset 姿态余量从 6.8 mm 降到 **1.8 mm**
# (reset 足端 X 半宽 0.0382, 墙心硬下界 = 0.0382 + 半厚 0.01 = 0.0482),
# 再往里就是"机器人一出生就嵌在墙里"。标定依据见技术细节 §7.14 / §7.15。
...
_CURVES: dict[str, tuple[float, ...]] = {
    ...
    # 2026-09-26 从 35.0 降到 2.0: 定量依据来自 run 2026-09-26_20-47-56 的收益账 ——
    # 0.080 档下成功脉冲贡献 +5.44/s, 而"卖掉姿态"只多付 0.43/s 的姿态代价, 即每单位
    # 脉冲权重值 5.44/35 = 0.155/s。要让卡墙解不再划算需要 w < 0.43/0.155 ≈ 2.8,
    # 取 2.0 留出余量。**不要设成 0 以外的改动**都安全: 该奖励项是 stand_reward_and_pulse()
    # 的唯一调用点(站立窗口推进/循环复位/脉冲产生都在里面), 但 RewardManager 跳过零权重项
    # 看的是 env_cfg 的 weight(恒 1.0)而非本曲线值, 所以本值取 0 也不会停掉那套机制。
    # 依据与实测见技术细节 §7.15。
