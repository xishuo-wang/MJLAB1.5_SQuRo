...
**Phase 1**：接近段(圆弧, 平台-K+过渡-K→0 反推) → **平滑 LUT** 绕杆路径。有直行模式一个周期 6 段：CW弧→CCW弧→直行→CCW弧→CW弧→直行，**每相邻杆之间均有等长直行段**（长度 = spacing−2×x_sw_full，S3 结束于杆2 x、S6 结束于杆3 x）；无直行模式 4 段同向连续。弧曲率 = ±CURVATURE_TARGET，且**所有曲率跳变线性过渡**（平滑弧长 = SMOOTH_VEL×SMOOTH_TIME，与 vel 解耦，几何固定）。弧长 = vel×t − _INIT_DIST（vel 固定），跨周期 x_ref 叠加偏移保证连续。

### 平滑 LUT 的双模式

- **有直行模式**：间距 ≥ 2×x_sw_full (0.1260)，弧↔直行均有过渡；**两段直行**（每段长度 = spacing−2×x_sw_full），分别位于杆1→杆2 之间（y=-2×x_sw_full）与杆2→杆3 之间（y=0），每根杆间直行段一致
