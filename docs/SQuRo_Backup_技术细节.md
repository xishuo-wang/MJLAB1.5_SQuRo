...
| **T2 回收** | 0.65 → 0.80 | +0.60 → 0 | **−1.57 保持** | +0.60 → 0 | **+1.57 保持** | 保持扭转、收回侧摆/俯仰 → 停在 S1 姿态 |
| **T3 解扭** | 0.80 → 0.95 | 0 → +0.60 | −1.57 → 0 | 0 保持 | +1.57 → 0 | 解开扭转 → **实际翻滚发生在此段** |
| **T4 过渡** | 0.95 → 1.45 | +0.60 → 0 | 0 | 0 | 0 | 腿从支撑角回站立角 |
| **T5 保持** | 1.45 → ∞ | 0 | 0 | 0 | 0 | 站立保持 |

**记忆要点**：`F_spine1` 走 **0 → 0.6 → 0 → 0.6 → 0** 双峰；`H_spine1` 只走 **0 → 0.6 → 0** 单峰；
两个扭转关节是**反相满量程**。
...
**判据已改为基于 §4 的标记 site（不再依赖四元数）。** `mdp/command.py`：

```python
# belly_z < back_z  ⇔  腹面朝下  ⇔  该段已翻正
def _segment_upright(self, idx):        # idx: 0=F 段, 1=H 段
    belly_id, back_id = _MODEL_INDICES.segment_belly_back_ids[idx]
    sp = self._asset.data.site_pos_w
    return sp[:, belly_id, 2] < sp[:, back_id, 2]

S1 = (~F_upright) & H_upright & (zF < 0.03) & (zH < 0.03)   # 只有后段翻正
S2 =   F_upright  & H_upright & (zF < 0.04) & (zH < 0.04)   # 两段都翻正
```
...
两个门控都留 `buffer = 0.3 s` 的缓冲窗；超时未达标则 `t_phase` 归零重试（最多 5 次）。

**判据与时序的一致性已验证**（`scripts/Backup/verify_gate_predicates.py`，两法独立重算，不一致帧数为 0）：
