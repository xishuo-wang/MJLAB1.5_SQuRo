...
半厚 0.01 m。因此**实际内侧净宽 = a − 0.02**：`a=0.20` 时净宽只有 0.18 m。
`mdp/entity.py` 的 `RestrictedSpaceEntity.clear_width` 是这个口径的唯一出处。

**课程语义**：`a` 在**单个 episode 内固定**；阶段一（iter 0~3000）不开碰撞，
阶段二从 3000 轮起按 `get_curriculum_corridor_width()` 收紧到 0.20（6000 轮到位）。
...
1. **`contype/conaffinity` 运行期修改无效。** mjwarp 在 `put_model` 时就把碰撞对列表固化。
   ...
   而编译期就置 0 的对照组接触恒为 0。
   ⇒ "阶段一关碰撞"**只能**用编译期 `contype=0` 实现。
2. **几何位置运行期修改无效。** 实测把墙的 `geom_pos` 从 ±0.20 平移到 ±0.10
   （并 `expand_model_fields` + 重建 CUDA graph）后，机器人放到新墙位接触数为 **0**。
   ⇒ "移动墙来收紧走廊"不可行；也**不要**指望 mocap/关节方案绕过它（已试过 slide-joint，
   在 SQuRo 场景里同样不产生接触）。
3. **`find_geoms` 返回的是实体局部下标**，不能直接索引 `model.geom_contype`
   （会静默改到别的几何上，症状就是"开关看着没生效"）。必须经 `indexing.geom_ids` 换算。

**因此的设计取舍**：`a` 与碰撞开关都是 `env_cfg` 的一部分，与 Slalom 的 `PoleEntity` 用法一致；
换 `a` 或切阶段都要**重建环境**。env_cfg 顶部两个常量控制：`ENABLE_RESTRICTED_SPACE`
（是否加入实体）、`RESTRICTED_SPACE_WIDTH`（`None` = 跟随课程；给数值 = 整个训练固定该 `a`）。

**验收**：`uv run python -B -m mjlab.scripts.Backup.verify_corridor_wiring`（带失败断言：
课程取值、墙位与净宽口径、编译期开碰撞必须产生接触、关碰撞必须为 0）。

**已知且尚未解决**：初态机器人横向占位 `|x|max = 0.1439 m`（躯干+头，腿只到 0.0788），
⇒ `a` 的理论下界 = 0.288 m。`a=0.20`（净宽 0.18）**在几何上装不下当前初态**，
这是阶段二要真正优化的目标（机器人必须在翻正过程中自己收进走廊），不是 bug。
