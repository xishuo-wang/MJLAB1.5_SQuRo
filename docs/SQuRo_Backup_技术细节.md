...
- 成功判定不变（仅 P3、前后段各自 u>0.9、各自高度>0.05 m、连续 0.5 实际秒），但 `stand` 终止改为 `time_out=True`：提前结束会截断后续密集奖励（3 s 站起约丢 126 分 vs 里程碑 35 分），按截断处理后价值估计会 bootstrap 到后续状态。
- `events.reset_model` 显式"无则先建、有则清零" `_stand_elapsed`，去掉"termination 必须先跑过一次"的隐式依赖。
- 回归入口：`uv run python -B -m mjlab.scripts.Backup.verify_backup_stage_rewards`（11 项，覆盖区间项非门控、站立进度门控与单调性、ctrlrange 超限成本含 scale/名称对齐、阶段单向、里程碑去重、确认时间/重置、参考端点与多速度）。
