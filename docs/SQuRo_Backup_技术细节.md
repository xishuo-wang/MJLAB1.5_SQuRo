...
**权重标定**：`weight_leg_pose = 2.0`。实测保持段 MSE≈0.41 时该步值 ≈ −0.81
（`Episode_Reward` 口径约 −1.6/s），与 `height`(4.87)、`mimic_pos`(9.38) 同量级但不压过它们。
回归用例强制 `w·0.41 < weight_height` 且 `< weight_mimic_pos`。
**不要同时改** `track_joint` 的归一化 / `mimic_pos` 权重 / L1/L2 —— 那是三个变量，
会失去归因（`weight_leg_pose` 是这一批唯一改动量）。

**运行时诊断**：只上报 `Data/leg_pose_rmse`。**力矩饱和率不做运行时上报** —— 真实模型的
关节列含浮动基，执行器列序与之不同，运行期对齐容易写错且写错会把训练打挂（本次踩过）；
该量在离线诊断里从回放 CSV 的 `*_torque` 列直接算。

**验收**：`verify_backup_stage_rewards` 新增 5 条（共 70 项）——
P3 门控（P1/P2 必须为 0）、二次性（误差×2 ⇒ 代价×4）、只平均腿关节（脊柱误差完全不影响）、
读实际角（只改指令时取值不变）、量级低于 `height`/`mimic_pos`。

**尚未验证 / 后续批次**（按顺序，每批只改一个变量）：

- [ ] 第一批（本项）：`weight_leg_pose = 2.0`，其余全不动。若 500 轮内 `Data/leg_pose_rmse`
      不降，说明梯度被饱和吃掉，转下一批。
- [ ] 第二批：若**姿态仍差且指令继续超限**，只提高 P3 的 `weight_leg_target`（1.0 → 2.0）。
...
| 脚本 | 用途 |
|---|---|
| `verify_backup_stage_rewards.py` | **主回归入口**：判据/参考/状态机/奖励的 CPU 单测（70 项，不建环境、不落盘） |
