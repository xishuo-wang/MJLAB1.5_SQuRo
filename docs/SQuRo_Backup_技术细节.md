...
## 整体架构与设计变更：阶段互斥奖励与站立闭环（2026-09-16，当前配置）

- 范围：阶段姿态奖励、目标成本、参考边界、最终成功判据与回放诊断；PPO、物理参数和观测/动作维度不变。
- 旧行为：S1/S2 区间奖励全阶段同时生效，P3 返回 S1 仍得分；2999/2900 策略已触发 S2，但随后重新扭转、回退 P2。
- 新数据流：当前阶段与背腹 site → 对应阶段姿态进度 → 奖励；P3 双段正置且分别达到站立高度 → 连续确认 → 成功终止。
- 阶段名称：内部 `phase=0/1/2` 分别为 P1/P2/P3。用户所称 S1/S2/S3 区间对应这三个执行阶段；S3 为完整站立目标。
- `progress_s1` 权重 3，仅 P1 生效，目标前倒后正；`progress_s2` 权重 3，仅 P2 生效，目标双段正置。
- 新增 `progress_s3` 权重 3，仅 P3 生效：`min(uF,uH).clamp(0,1) × ((min(zF,zH)-0.024)/(0.055-0.024)).clamp(0,1)`。
- S3 使用较低一段的高度，不允许仅抬高一端冒充站立；该值是连续引导，不等同于成功判据。
- `spine_target` 修复遗漏的课程权重读取，四脊柱等权系数实际从 1 恢复为配置值 2。
- 新增 `leg_target` 权重 1，仅 P3 生效，跟踪腿部支撑到站立的当前参考，抑制极端支撑指令。
- 目标成本使用 RL 裁剪之后、XML 控制限幅之前的目标；没有改 PPO 分布或 ±6 裁剪，不能据此声称已消除所有原始 action 饱和。
- 参考查询为 P2 独立保留 T3 左端极限：末端侧摆 +0.6、其他脊柱 0，缓冲期速度全零；P3/T4 仍从全零脊柱开始。
- 分段节点速度改取右侧导数，避免把 T3→T4 的侧摆跳变当成 P3 起点速度；float32 时间和再次限幅，防止越界插值。
- 最终成功：仅 P3，前后段各自背腹方向余弦 >0.9、各自躯干高度 >0.05 m，连续 0.5 实际秒；不再使用基座重力投影和平均高度。
- 手调脚本共用站立几何；训练站立计时在 episode reset 时按环境清零，中断候选或退出 P3 即清零。
- S1/S2 仍为 45°、0.10 s，双倒回退仍为 0.15 s，P1/P2 缓冲仍为 1.0/0.3 s；里程碑仍每回合去重，不移除姿态回退。
- 策略 CSV 新增阶段、阶段时间、前后段方向余弦与高度、确认/转移/回退/重试字段；`backup_phase` 是本步更新后阶段，确认时钟在切换时清零，须结合事件列解释。
- 保留 dt=0.002×5=0.01 s、rollout=96、PPO 5 epochs/4 minibatches、原平滑权重；当前速度课程为 [2,4] → [1,4]，不是下文历史记录中的 [3,6]。
- 回归入口：`uv run python -B -m mjlab.scripts.Backup.verify_backup_stage_rewards`，覆盖互斥奖励、双段站立、确认时间/重置、参考端点、多速度、成本权重、回退和里程碑去重。
- 验证边界：小批量仿真和短 PPO 更新只能验证代码链路，不能代替完整训练成功率评估；建议新训练作为干净对照，旧策略仅用于诊断。
