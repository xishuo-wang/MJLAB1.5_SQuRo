...
1. **判据加稳定性条件**：新增 14 个驱动关节速度的瞬时 RMS 门限
   （进入 `STAND_VEL_RMS_ENTER = 3.5`，退出 `STAND_VEL_RMS_STAY = 5.5` rad/s）。
...
3. **站立确认专用迟滞**（`BackupCommand._update_stand_confirmation`，**不动**共享的
   `_update_confirmation`，那套是 S1/S2/倒置用的硬清零语义）：
   严格进入 `enter` 推进累计；只有稍宽的 `stay` 时按 `STAND_CONFIRM_DECAY = 2.0`
   **主动侵蚀**累计；两者都不满足立即清零；结算当步必须仍在 `enter` 上。

**为什么用"主动侵蚀"而不是"暂停累计"**：暂停累计会让
"达标 0.4 s → 掉出 0.1 s → 达标 0.2 s"拼接成一次 0.6 s 的连续站稳。
侵蚀 2 倍后同样的 0.1 s 掉出要扣 0.2 s，短暂达标无法拼接；
而单次瞬时抖动只损失 2 倍于该抖动的时长，不会像硬清零那样一票否决。

**验收方法**：`verify_backup_stage_rewards.py` 新增
`test_stand_requires_low_joint_velocity`（迟滞带只侵蚀不累计、超退出阈值立即清零）与
`test_stand_dropout_erodes_instead_of_pausing`（侵蚀到 0 后必须重新攒满 1.5 s）；
另外 `SQuRo_backup_Replay.py --visualize none` 的 `[RESULT] phase=DONE`
读的就是**训练环境**的 `stand` 终止项，因此手调版本通过即证明新判据可达。

**调参入口**：`Progress/stand_vel` 记录"站立几何成立时的关节速度 RMS 均值"
（不参与判据），下一轮训练直接看它离 3.5 还有多远再决定收紧还是放松。
...
若 σ 已经很低而 `Progress/stand_vel` 仍高于门限，才是真正的"站不稳"，此时才该放松门限；
