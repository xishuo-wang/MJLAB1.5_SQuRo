...
**待确认（2026-09-21 复查发现）**：上面写的"P0 就是相位 0 时钟的前 `0.50λ` 秒"与代码
**不一致**。实测 `get_reference_joint_state`（λ=1，phase=0）：
...
上面"原设计让参考在 T1 一开始就直接输出支撑角"的动机并未被消除。原因是
`reference._stage_t_nom` 里写的是 `P1_ONSET + τ.clamp(max=P1_SPAN)`（表时间 `0.50+τ`），
而 §5.4 的意图要求 `τ.clamp(max=P1_END)`（表时间 `= τ`，`[0,1.30]` 正好是 P0+T1+T2）。
两者在**推进时刻**上等价（都是 `1.30λ`），差别只在"这 1.30λ 秒里参考在演什么"：
当前实现把 T1 压进前 `0.80λ` 秒、后 `0.50λ` 秒冻结在 S1 姿态；意图实现则是
P0 占前 `0.50λ`、T1 后移 `0.50λ`。改哪一边都会动到 P1 的实际跟踪内容，故**待用户裁定后再改**。
